<?php
    ...
    use Simplon\Payment\ExceptionConstants;
        ...
        protected static function _processResponse($response)
        {
            if ($response)
            {
                ...
                if (isset($response['error']))
                {
                    ...
                    {
                        $code = ExceptionConstants::ERR_API_CODE;
                        $message = ExceptionConstants::ERR_API_MESSAGE;
                        unset($error['type']);
                    }

                    elseif ((string)$error['type'] === 'invalid_request_error')
                    {
                        $code = ExceptionConstants::ERR_REQUEST_CODE;
                        $message = ExceptionConstants::ERR_REQUEST_MESSAGE;
                        unset($error['type']);
                    }

                    elseif ((string)$error['type'] === 'card_error')
                    {
                        $code = ExceptionConstants::ERR_PAYMENT_DATA_CODE;
                        $message = ExceptionConstants::ERR_PAYMENT_DATA_MESSAGE;
                        unset($error['type']);
                    }

                    else
                    {
                        $code = ExceptionConstants::ERR_UNKNOWN_CODE;
                        $message = ExceptionConstants::ERR_UNKNOWN_MESSAGE;
                    }
